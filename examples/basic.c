...
int main(int argc, char *argv[])
{
	...
	shader *S = shader_loadFromFile("src/glsl/shader.vert",
					"src/glsl/shader.frag", PHONG);
	shader *textShader = shader_loadFromFile(
		"src/glsl/textShader.vert", "src/glsl/textShader.frag", TEXT);
	...
	lightColor[2] = 0;
	pointLight *point =
		initPointLight(lightColor, init_cam_position, 2.0f, 2.0f);
	setPointLight(point, S); // S is the shader

	level *mainLevel =
		level_loadMeshes("res/maps/map.bmp", "res/textures/metal.jpg");
	list_insert(mainLevel->meshList,
		    mesh_genFlatFloor(
			    mainLevel->size,
			    "res/textures/grass.jpg")); // CHECK FOR ERRORS

	linkedList *meshList = list_create();
	mesh_loadFromFileToList("res/obj/spot.obj", "res/textures/spot.png",
				meshList);

	linkedList *bulletList = list_create();
	bulletType *bulletSpecs = bullet_createType(
		0.01, 100, 20, "res/obj/bullet.obj", "res/textures/metal.jpg");
	...
	movement->forward = movement->backward = movement->right =
		movement->left = false;
	bool show_fps = false;
	...
	while (running) {
		...
		if (show_fps &&
		    currentTime - initial_frame > UPDATE_FPS_THRESHOLD) {
			char fps_buffer[20];
			unsigned int fps = (int)(frames_passed * 1000.0f) /
					   (currentTime - initial_frame);
			if (fps > 999)
				fps = 999;
		...
		while (SDL_PollEvent(&e)) {
			...
			if (e.type == SDL_MOUSEBUTTONDOWN)
				list_insert(bulletList,
					    bullet_create(C->eye, C->direction,
							  bulletSpecs));
			if (e.type == SDL_KEYDOWN)
				...
				case SDLK_ESCAPE:
					...
					SDL_WarpMouseInWindow(window, mouse_x,
							      mouse_y);
					break;
				case SDLK_q:
					running = false;
		...
		while (current_node) {
			...
			if (bullet_maxDistance(b)) {
				bullet_destroy(b);
				current_node = list_delete_node(bulletList,
								current_node);
				continue;
			}
