...
int main(int argc, char* argv[]) {
	...
	text *title = text_create("C3D", pixFont, 5, 0.45f, 0.90f);
	text *wall = text_create("Map loading/generation (kinda buggy)", pixFont, 2.5, 0.2f, 0.85f);
	...
	while (running) {
		...
		text_draw(title, ortho, textShader);
		text_draw(wall, ortho, textShader);
